...
const FileUpload: React.FC<FileUploadProps> = ({ onUpload, isProcessing }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-slate-100 p-8 mb-10">
      ...
      {activeTab === 'upload' ? (
        <div
          className={`relative border border-dashed rounded-xl h-64 flex flex-col items-center justify-center transition-all duration-300 group ${dragActive
...
const TabButton = ({ isActive, onClick, label }: any) => (
  <button
    onClick={onClick}
    className={`px-4 py-1.5 text-xs font-medium rounded-md transition-all duration-200 ${isActive
